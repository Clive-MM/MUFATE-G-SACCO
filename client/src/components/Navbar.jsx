...
const Navbar = () => {
  ...
  const NavDropdown = ({ label, items, isActive, to }) => {
    ...
    return (
      <Box onMouseEnter={() => setIsOpen(true)} onMouseLeave={() => setIsOpen(false)} onFocus={() => setIsOpen(true)} onBlur={() => setIsOpen(false)} sx={{ position: 'relative' }}>
        <Link component={RouterLink} to={to} underline="none" sx={sharedLinkStyles(isActive)} aria-expanded={isOpen}>
          {label}
  ...
  return (
    <AppBar position="fixed" elevation={0} sx={{ background: 'rgba(2, 21, 15, 0.96)', borderBottom: `1px solid ${getAlphaGold(0.2)}`, zIndex: theme.zIndex.appBar }}>
      <Toolbar sx={{ justifyContent: 'space-between', py: { xs: 1, md: 1 }, px: { md: 4 }, minHeight: { xs: 70, md: 100 } }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Link component={RouterLink} to="/" underline="none" sx={{ display: 'flex', alignItems: 'center', color: 'inherit' }}>
            <Box component="img" src="https://res.cloudinary.com/djydkcx01/image/upload/v1764080163/ChatGPT_Image_Nov_25_2025_05_15_43_PM_kt0vz9.png" alt="Logo" sx={{ height: { xs: 50, md: 75 }, width: 'auto' }} />
            <Stack spacing={0} sx={{ ml: 1.5 }}>
              <Typography sx={{ fontSize: { xs: '0.75rem', md: '1rem' }, fontWeight: 900, color: BRAND_GOLD, textTransform: 'uppercase' }}>Golden Generation</Typography>
              <Typography sx={{ fontSize: { xs: '0.75rem', md: '0.9rem' }, fontWeight: 900, color: BRAND_GOLD, textTransform: 'uppercase' }}>DT SACCO</Typography>
            </Stack>
          </Link>
        </Box>
        ...
        {!isMobile ? (
          <Stack direction="row" spacing={2.5} alignItems="center">
            ...
            <Button component={RouterLink} to="/customer_registration" variant="contained" sx={premiumButtonStyle}>Register</Button>
            <Tooltip title="Contact Us" arrow sx={tooltipStyles}>
                <IconButton onClick={handleContactClick} component={motion.button} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} sx={{ color: BRAND_GOLD, border: `2px solid ${BRAND_GOLD}`, ml: 2 }}>
                    <PhoneIcon fontSize="small" />
                </IconButton>
            </Tooltip>
          </Stack>
        ) : (
          <IconButton onClick={() => setDrawerOpen(true)} sx={{ color: BRAND_GOLD }} aria-label="menu"><MenuIcon fontSize="large" /></IconButton>
        )}

        {/* MOBILE DRAWER */}
        <Drawer anchor="left" open={drawerOpen} onClose={() => setDrawerOpen(false)} sx={{ '& .MuiDrawer-paper': { width: '280px', backgroundColor: BRAND_DARK, color: BRAND_TEXT_LIGHT } }}>
          ...
          <List sx={{ px: 1 }}>
            {navLinks.map((item) => (
              <React.Fragment key={item.label}>
                {item.items ? (
                  <>
                    <ListItemButton onClick={() => setMobileOpenMenu(mobileOpenMenu === item.label ? '' : item.label)}>
                  ...
                  <ListItemButton component={RouterLink} to={item.to} onClick={() => setDrawerOpen(false)} sx={{ backgroundColor: location.pathname === item.to ? getAlphaGold(0.1) : 'transparent' }}>
...
// CRITICAL FIX: The export statement below was missing
