...
const MemberRegistration = () => {
  ...
  // AUTOSAVE: Load from localStorage on init
  const [formData, setFormData] = useState(() => {
    const saved = localStorage.getItem("mufate_reg_draft");
  ...
  // AUTOSAVE: Save to localStorage whenever formData changes
  useEffect(() => {
    localStorage.setItem("mufate_reg_draft", JSON.stringify(formData));
  }, [formData]);
  ...
  const isStepValid = () => {
    ...
    if (activeStep === 0) {
      const { FullName, Salutation, IDType, IDNumber, DOB, MaritalStatus, Gender } = formData;
      if (!FullName || !Salutation || !IDType || !IDNumber || !DOB || !MaritalStatus || !Gender) return false;
    ...
    if (activeStep === 1) {
      const { County, District, MobileNumber } = formData;
      if (!County || !District || !MobileNumber) return false;
      if (!phonePattern.test(MobileNumber)) return false;
    }

    if (activeStep === 2) {
      const { NomineeName, NomineeIDNumber, NomineePhoneNumber, NomineeRelation } = formData;
      if (!NomineeName || !NomineeIDNumber || !NomineePhoneNumber || !NomineeRelation) return false;
      if (!phonePattern.test(NomineePhoneNumber)) return false;
    }
  ...
  const confirmSubmission = async () => {
    ...
    try {
      ...
      setSuccess(true);
      localStorage.removeItem("mufate_reg_draft"); // Clear autosave on success
  ...
  const renderField = (field) => {
    ...
    // Check for phone format errors to show helper text
    const phonePattern = /^2547\d{8}$/;
    const isPhoneField = ["MobileNumber", "NomineePhoneNumber", "AlternateMobileNumber"].includes(field);
    const hasPhoneError = isPhoneField && formData[field] && !phonePattern.test(formData[field]);
    ...
    return (
      <TextField
        fullWidth
        variant="filled"
        required={!["KRAPin", "AlternateMobileNumber", "Email", "Profession", "ProfessionSector", "Division", "Address", "PostalCode", "PhysicalAddress"].includes(field)}
        name={field}
        label={field}
        value={formData[field] || ""}
        onChange={handleChange}
        ...
        error={hasPhoneError}
        helperText={hasPhoneError ? "Use format 2547XXXXXXXX" : ""}
        ...
        <AnimatePresence mode="wait">
          {!success ? (
            <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <Typography align="center" sx={{ fontWeight: 900, color: BRAND.gold, fontSize: { xs: '1.8rem', md: '2.5rem' }, textTransform: "uppercase", letterSpacing: "4px", mb: 4, textShadow: `0 0 15px ${BRAND.gold}33` }}>
                Member Registration
              </Typography>
              ...
              <Box sx={{ mb: 8 }}>
                <Stepper activeStep={activeStep} alternativeLabel sx={{ "& .MuiStepConnector-line": { borderColor: "rgba(255,255,255,0.05)" } }}>
                  {steps.map((s, idx) => (
                    <Step key={s.label}>
                      <StepLabel StepIconProps={{ sx: { color: activeStep >= idx ? BRAND.gold : "rgba(255,255,255,0.1)", "&.Mui-active": { color: BRAND.gold, filter: `drop-shadow(0 0 10px ${BRAND.gold}66)` } } }}>
              ...
              <Stack direction="row" spacing={2} justifyContent="center" sx={{ mt: 8 }}>
                {activeStep > 0 && (
                  <Button variant="text" sx={{ color: BRAND.textMuted, px: 4 }} onClick={() => setActiveStep((prev) => prev - 1)}>
                    Back
                  </Button>
                )}
                ...
                <Button
                  sx={refinedGlowBtn}
                  disabled={loading || !isStepValid()} // Disable until valid
